	...
	const char* name, vec3 position, float radius, entity_t** out)
{
	...
	entity_init_common(name, ENTITY_EXPLOSION, position, &entity);
...
void entity_tick_explosion(sbox_t* sbox, entity_t* entity, entity_explosion_t* explosion) {
    r_add_partfx_explode(sbox, &sbox->renderer, entity->position, explosion->radius);
    ...
    for (int i = 0; i < MAX_PLAYERS; i++) {
        ...
        if (dist < explosion->radius) {
            ...
            glm_vec3_scale(dir, lerp(2.0f, 5.0f, explosion->radius - dist), dir);
