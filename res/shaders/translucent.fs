...
void main() {
    ...
    vec3 normal = perturb_normal(vs_normal, view_dir, uv);
    vec4 albedo = texture(materials[vs_mat].albedo, uv);
    float roughness = texture(materials[vs_mat].roughness, uv).r;
    float ao = 1.0f;

    vec3 ambient = vec3(0.4) * albedo.rgb * ao;
    
    frag_color = vec4(ambient, albedo.a);
}
