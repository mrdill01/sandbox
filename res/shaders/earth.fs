...
vec3 perturb_normal(vec3 normal, vec3 view_dir, vec2 uv) {
    ...
    vec3 map = texture2D(materials[0].normal, uv).xyz;
    map.y = 1.0f - map.y;
...
void main() {
    vec2 uv = vs_uv * vec2(materials[0].tilex, materials[0].tiley);
    uv += vec2(materials[0].scrollx, materials[0].scrolly);
    ...
    sample.albedo = texture(materials[0].albedo, uv);
    sample.albedo.rgb = pow(sample.albedo.rgb, vec3(GAMMA));
    sample.roughness = texture(materials[0].roughness, uv).r;
